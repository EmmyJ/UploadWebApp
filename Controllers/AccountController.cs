    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            if (!string.IsNullOrEmpty(model.UserName) && !string.IsNullOrEmpty(model.Password))
            {
                ...
                {
                    if (model.Password == user.pwd)
                    {
                        FormsAuthentication.SetAuthCookie(model.UserName, model.RememberMe);

                        FormsAuthenticationTicket authTicket = new FormsAuthenticationTicket(1, user.name, DateTime.Now, DateTime.Now.AddMinutes(180), false, user.name);
            ...
            // If we got this far, something failed, redisplay form (or send failure code)
